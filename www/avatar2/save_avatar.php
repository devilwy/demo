...
//**2012 04 04 修改
//$arr = explode('_',$pic_id);
//$uid = isset($arr[1])?intval($arr[1]):1;
$ext = strrchr($orgin_pic_path,'.');

$md5userid = md5($pic_id);
$new_avatar_path = $_SERVER['DOCUMENT_ROOT']."/upload/".substr($md5userid,0,2).'/'.substr($md5userid,2,2).'/'.$pic_id.'_'.$type.$ext;
...
$d->data->urls[0] = "http://".$_SERVER['HTTP_HOST'].'/upload/'.substr($md5userid,0,2).'/'.substr($md5userid,2,2).'/'.$pic_id.'_'.$type.$ext;
$d->status = 1;
$d->statusText = '上传成功!';
